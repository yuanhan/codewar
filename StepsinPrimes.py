# Steps in Primes
# 6kyu
# https://www.codewars.com/kata/steps-in-primes/train/python
...
def divisors2(n):
    for i in range(2,n): # it is slow in here
        if n % i == 0:
            return True 
...
def step(g, m, n):
    ...
    for i in range(len(primes)):
        if primes[i] + g in primes:
            return [primes[i], primes[i]+g]

---------------------------------------------
def step(step, start, stop):
    for n in xrange(start, stop + 1):
        if isPrime(n) and isPrime(n + step):
            return [n, n + step]
    return None

def isPrime(num):
    if num > 2 and num % 2 == 0: # this is why it is fast
        return False
    for x in range(3, int(num**0.5 + 1), 2): # this is why it is fast
        if num % x == 0:
            return False
    return True
